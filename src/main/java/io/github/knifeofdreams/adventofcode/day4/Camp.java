...
public class Camp {
  ...
  public long numberOfFullOverlaps(List<List<Section>> pairs) {
    return pairs.stream()
        .map(pair -> {
  ...
  public long numberOfPartialOverlaps(List<List<Section>> pairs) {
    return pairs.stream()
        .map(
            pair -> {
              var section1 = pair.get(0);
              ...
              return (section1.end >= section2.beginning && section1.beginning <= section2.beginning)
                  || (section2.end >= section1.beginning && section2.beginning <= section1.beginning);
            })
        .filter(overlaps -> overlaps)
        .count();
  }
